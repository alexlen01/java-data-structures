...
import static com.local.coding.primitivetypes.ReverseBits.DataType;
...
public class ReverseBitsTest {
	...
	public void test_reverseBit_8() {
		...
		for (int i = 0; i < bytes.length; i++) {
			...
			long reversed = ReverseBits.reverse(number, DataType.BYTE);
	...
	public void test_reverseBit_32() {
		...
		for (int i = 0; i < bytes.length; i++) {
			long number = bytes[i];
			assertNotEquals(number, 0l);
			long reversed = ReverseBits.reverse(number, DataType.INTEGER);
			assertNotEquals(reversed, 0l);
			String binary_initial = Long.toBinaryString(number);
